...
impl StatusCode {
    ...
    pub fn as_str(&self) -> &str {
        ...
        unsafe {
            std::str::from_utf8_unchecked(&CODE_DIGITS[offset..offset+3])
        }
    }
...
// A packed array of 3-ASCII-digit status code values for the supported range
// of (100..=999)
const CODE_DIGITS: [u8; 3*900] = {
    let mut codes = [0u8; 3*900];
    let mut s = 100;
    let mut i = 0;
    while s <= 999 {
        let (h, r) = (s / 100, s % 100);
        let (t, o) = (r /  10, r %  10);
        codes[i  ] = b'0' + h as u8;
        codes[i+1] = b'0' + t as u8;
        codes[i+2] = b'0' + o as u8;
        i += 3;
        s += 1;
    }
    codes
